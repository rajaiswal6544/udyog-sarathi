import React from 'react'
import './Jobsearch.css';
export default function Jobsearch(props) {
  return (
      <div className='searchjobmaindiv21'>
                <div className='searchjobstyle21'>
                    <h1 className='searchjobtitle21'>{props.title.split(" ").slice(0, 5).join(" ") + "..."}</h1>
                    <div className='searchlocation21'>
                        <p className='searchcompany21' >{props.company}</p>
                        <div style={{display:'flex',gap:"4px"}}>
                            <div><img style={{width:"16px",height:"16px",mixBlendMode:"multiply",borderRadius:"20px",marginTop:"2px"}}src="data:image/png;base64," alt="location" /></div>
                            <div><p className='searchplace21'>{props.place}</p></div>
                        </div>
                    </div>
                    <p className='searchdesc21'>{props.desc.split(" ").slice(0, 8).join(" ") + "..."}</p>
                    <h3 className='searchsalary21'>₹ {props.minsal}k - {props.maxsal}k</h3>
                    <div className='searchapply21'>
                        <div style={{display:"flex",gap:"15px",marginTop:"13px"}}>
                            <div><button className='searchapplybutton21'>Fulltime</button></div>
                            <div><button className='searchapplybutton21'>Remote</button></div>
                        </div>
                        <div><button className='searchviewbutton21'>View now</button></div>
                    </div>
                </div>
            </div>
  )
}
